...
export async function closeEditorsOutsideWorktree(
  ...
  try {
    ...
    log(
      "close editors outside worktree: inspected tabs",
      inspectedTabs.map(({ tab: _tab, ...entry }) => entry),
    );

    const closeCandidates = inspectedTabs.filter(
      (entry) => entry.outsideSelectedWorktree,
    );
    ...
    log(
      "close editors outside worktree: closing candidates",
    ...
    log("close editors outside worktree: done", {
