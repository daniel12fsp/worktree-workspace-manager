...
export function normalizeConfiguredRepositoryPath(fsPath: string): string {
  ...
  if (fsPath.endsWith(".git")) {
    ...
    if (fs.existsSync(path.join(repoRoot, ".bare"))) {
      log("normalized legacy .git config path to .bare repo root", {
        configPath: fsPath,
        repoRoot,
      });
      return repoRoot;
...
> {
    ...
    repos.map(async (repo) => {
      try {
        ...
        void vscode.window.showWarningMessage(
          `Failed to list worktrees for ${repo.configPath}: ${String(error)}`,
        );
  ...
  void ensureConfiguredColorsForWorktrees(all).catch((error) =>
    logError("failed to populate worktree colors", { error }),
  );
  return all;
}
...
export function colorKeyForWorktree(
  repo: BareRepository,
  name: string,
): string {
...
export async function ensureConfiguredColorsForWorktrees(
...
export function configuredColors(): WorktreeColors {
  const raw = vscode.workspace
    .getConfiguration("worktreeManager")
    .get<Record<string, unknown>>("colors", {});
